...
#closer look to top 3 with the most negative tweets
neg_data %>% 
...
#closer look to top 3 with the most negative tweets detail of the differences
neg_data %>% 
  filter(airline %in% c('US Airways', 'American', 'United')) %>%
  ggplot(aes(x = reorder(airline, -perc_neg), fill = airline, y = perc_neg)) +
  geom_col() +
...
#Example for waterfall chart

#Produce a toy dataset
...
#Scatter plot and regression lines with the diamonds dataset
...
#First simple fix- zoom in a smaller region
ggplot(diamonds, aes(carat, price)) + 
  geom_point() +
  geom_smooth(method = "lm") + 
  ylim(0, 20000) + xlim(0, 3.5)

#Add categorical feature (color) to get insights
...
#Add categorical feature (cut) to get insights
...
# To highlight the "Fair" class, can combine all the other classes
diamonds %>%
...
# Supplier Market Share for pie chart example
...
#Economics example for a plot with a secondary y-axis

#Plot with a secondary y-axis
economics %>% 
...
#Combined plots without showing secondary y-axis
economics %>% 
  filter(year(date) %in% c(2005:2010), month(economics$date) == 12) %>% 
  mutate(year = year(date)) %>% 
  ggplot() +
...
#Example of Slopegraph with employee feedback
...
# Example of clustered plot with bnames dataset
